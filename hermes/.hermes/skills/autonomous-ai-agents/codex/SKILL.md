...
There is no `codex` subcommand that prints ChatGPT plan usage/rate-limit numbers directly (checked `--help`, `doctor`, `features`; none expose it). The interactive TUI has `/status` and `/usage` slash commands, but they render into the alt-screen TUI rather than plain stdout, so capturing exact percentages via `process_manage` log output is unreliable — the TUI redraws over itself and only a status line like "You have N usage limit resets available" survives to a text poll. Don't present scraped TUI fragments as authoritative numbers; if precise quota is needed, tell the user to run `codex` interactively themselves and check `/status`.
