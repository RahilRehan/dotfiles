...
`claude -p '/usage'` prints session and weekly quota stats non-interactively (subscription usage, resets, request/session counts, top contributing skills/MCP servers) — no tmux or interactive session needed. This is the fastest way to check "how much Claude Code quota is left" for this user.

### tmux-unavailable fallback (Hermes terminal tool)
If `tmux` is not installed in this environment, the Hermes `terminal`/`process_manage` tools ARE a full substitute for tmux orchestration: `terminal(background=true, pty=true)` starts the interactive TUI, `process_manage(action="poll"/"log")` replaces `tmux capture-pane`, and `process_manage(action="submit", data=...)` (appends Enter) or `action="write"` (raw bytes, e.g. `\x15` for Ctrl+U) replaces `tmux send-keys`. Don't assume tmux is present — check with `which tmux` first and fall back to `process_manage` rather than failing the task.
